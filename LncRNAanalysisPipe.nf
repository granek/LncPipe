...
//Prepare annotations
  ...
  //  if (!KnownLncRNAgtf.exists()) exit 1, print_red("In non-human mode, known lncRNA GTF annotation file not found: ${params.known_lncRNA_gtf}")
	...
	# Trying to fix problem in Summary_renaming_and_classification
	# In that step gencode.protein_coding.gene.bed is generated.
	# The first step in generating it is to extract lines from known_coding.gtf with feature "gene" from 
	# however in the commented line below, known_coding.gtf is generated by REMOVING all "gene" features
	# not at all sure if this hack will have other side effects!!!
	...
	perl -lpe 's/ ([^"]\\S+) ;/ "$1" ;/g' !{known_coding_gtf} | grep -w gene -v > known_coding_nogenes.gtf 
	cat know_lnc.gtf known_coding_nogenes.gtf  > non_human_mod.gtf
	cat !{known_coding_gtf} > known_coding.gtf
...
if(design!=null){
    ...
    if(!params.merged_gtf) {
	    ...
	    Rscript -e "library(fs); dir_copy(system.file(package = 'LncPipeReporter'), getwd())"
